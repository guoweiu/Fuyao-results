...
hosts = ["inter"]
qpses = [["f10", "f20", "f40", "f60", "f80"], ["f10", "f20", "f40", "f60", "f80"]]
...
for host, qpss in zip(hosts, qpses):
    for qps in qpss:
        ...
        for worker_index in range(workers):
            start_file_path = "Pheromone/%s/%s/exp09StartFinra_worker_%d.stdout" % (host, qps, worker_index)
            end_file_path = "Pheromone/%s/%s/exp09End_worker_%d.stdout" % (host, qps, worker_index)
        ...
        # start_timestamp = sort_start_times[0]
        ...
        try:
            ...
            print("pheromone-%s-%s: %d, %d, %d" % (host, qps, min_lat, median_lat, tail_lat))
            # print(sorted(lats))
