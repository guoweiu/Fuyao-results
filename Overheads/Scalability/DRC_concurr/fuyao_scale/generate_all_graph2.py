...
#two_list=["40-20-1"]
data_list=[]
labels=["100","200","400","800","1200","1600","2000"]

for i, file in enumerate(labels):
    data = np.loadtxt("./"+file+".log",dtype=np.int64)
    # if file in two_list:
    ...
    for d in data:
        ...
        data_list.append([int(labels[i]), d])
...
cols = ["DRC numbers", r"Transfer latencty (${\rm \mu}$s)"]
# cols = ["DRC numbers", "Latencty (ms)"]

df = pd.DataFrame(data=data_list, columns=cols)

print(df.groupby("DRC numbers")[r"Transfer latencty (${\rm \mu}$s)"].describe())

fig = plt.subplots(figsize=(2.325, 1.8), dpi=300)

plt.subplots_adjust(hspace=None, wspace=0.2, top=0.99, bottom=0.24, left=0.21, right=0.99)

plt.grid(zorder=1, linestyle="--", alpha=0.3)  # 显示网格
# plt.grid(zorder=1, linestyle="--", alpha=0.3)  # 显示网格

ax = sns.boxplot(x="DRC numbers", y=r"Transfer latencty (${\rm \mu}$s)", data=df, linewidth=0.8, showfliers=False, width=0.5)

for box in ax.artists:
    box.set_facecolor("#acc4ed")

plt.xticks(fontsize=9, rotation=30)
plt.gca().xaxis.set_tick_params(pad=0)
plt.gca().yaxis.set_tick_params(pad=0)

ax.set_xlabel("DRC numbers", labelpad=0)
