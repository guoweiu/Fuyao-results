...
for file_path in files:
    ...
    with open(file_path_c, 'r') as file:
        for line in file:
            if 'lgw' not in line:
                continue
            if 'manager' in line or 'coordinator' in line or 'scheduler' in line:
                parts = line.split()
                index_value = ''
                if 'S' in parts:
                    index_value = 'S'
                ...
                elif 'coordinator' in line or 'scheduler' in line:
                    cpu_percentage = float(parts[cpu_percentage_index]) * 2
