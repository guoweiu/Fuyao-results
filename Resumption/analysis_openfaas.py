...
for file_path in files:
    ...
    file_path_c = abs_path + '/' + 'controller' + '/' + file_path
    cpu_total = 0.0
    with open(file_path_c, 'r') as file:
        for line in file:
            if 'gateway' in line or 'faas-netes' in line:
                parts = line.split()
                index_value = ''
                if 'S' in parts:
                    index_value = 'S'
    ...
    file_path_f = abs_path + '/' + 'fn' + '/' + file_path
    cpu_total = 0.0
    with open(file_path_f, 'r') as file:
        for line in file:
            if 'systemd+' not in line:
                continue
            if 'python' in line or 'fwatchdog' in line:
                parts = line.split()
                index_value = ''
                if 'S' in parts:
                    index_value = 'S'
